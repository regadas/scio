...
object CheckBeamDependencies extends AutoPlugin {
  ...
  object autoImport {
    val checkBeamDependencies = taskKey[Unit]("check")
    val depsTask = TaskKey[Unit]("finds the dependencies")
  }
  import autoImport._

  val beamDeps = new ConcurrentHashMap[String, Map[String, Set[String]]]()

  def resolveBeamDependencies(deps: Seq[(String, String)]): Map[String, Set[String]] =
    deps
      ...
      .map {
        case (orgName, rev) =>
          beamDeps.computeIfAbsent(
            orgName,
            new function.Function[String, Map[String, Set[String]]] {
